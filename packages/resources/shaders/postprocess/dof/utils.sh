...
#define linear_depth(z) \
    ((u_proj[3][3] == 0.0) ? \
        (u_near_far.x * u_near_far.y) / (z * (u_near_far.x - u_near_far.y) + u_near_far.y) : \
        z * (u_near_far.y - u_near_far.x) + u_near_far.x) /* Only true for camera view! */
