...
float coneTraceOcclusion(in ConeTraceSetup setup, const highp sampler2D depthTexture) {
    ...
    highp vec2 ssEndPos = (mul(setup.screenFromViewMatrix, vec4(vsEndPos, 1.0))).xy * ssEndPosWInv;
    ...
    for (uint i = 0u; i < setup.sampleCount; i++, t += dt) {
        ...
        float vsSampleDepthLinear = -depthVS_from_texture(depthTexture, ssSamplePos * setup.resolution.zw, 0.0);
