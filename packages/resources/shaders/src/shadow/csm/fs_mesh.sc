...
float compute_csm_visible(ivec4 selections, out vec3 color_coverage)
{
	if (selections[0])
	{
...
void main()
{
	...
	float visibility	= compute_csm_visible(selections, color_coverage);
	...
	vec4 scenecolor	= saturate(ambientcolor + calc_lighting_BH(normal, v_lightdir, v_viewdir, lightcolor, 
															basecolor, u_specularColor, gloss, u_specularLight.x));
	vec4 finalcolor = min(u_shadow_color, scenecolor, visibility);

	gl_FragColor.xyz = mix(fogColor, finalcolor, fog_factor);
	gl_FragColor.w = 1.0;
}
