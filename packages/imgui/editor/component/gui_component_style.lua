...
function GuiComponentStyle:on_update()
    ...
    cursor.SameLine()
    if widget.Button("Generate Default Value") then
        self:generate_default_value()
    end
...
local DefaultValuePath = "engine_data/default_value_component.lua"
function GuiComponentStyle:generate_default_value(path)
    path = path or DefaultValuePath
    log.info_a("generate_default_value to:"..path)
    local tbl = ComponentSetting.create_default_value_dic()
    local tbl_str = gui_util.data_to_lua_file(tbl,"\t")
    local f = gui_util.open_current_pkg_path(path,"wb")
    local c <close> = setclose(function()
        f:close()
    end)
    f:write("--This file is auto generated by editor,do not modify!\n")
    f:write(tbl_str)
end
