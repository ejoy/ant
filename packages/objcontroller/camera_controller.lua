...
function camera_controller_system:init()	
	...
	objctrller.bind_tigger("move_forward", function (event)
		camera_util.move(camera, 0, 0, calc_step(speed_persecond, timer.delta * 0.001))
	end)
	objctrller.bind_tigger("move_backward", function (event) 
		camera_util.move(camera, 0, 0, -calc_step(speed_persecond, timer.delta * 0.001))
	end)
	objctrller.bind_tigger("move_left", function (event) 
		camera_util.move(camera, -calc_step(speed_persecond, timer.delta * 0.001), 0, 0)
	end)
	objctrller.bind_tigger("move_right", function (event) 
		camera_util.move(camera, calc_step(speed_persecond, timer.delta * 0.001), 0, 0)
	end)
	objctrller.bind_tigger("move_up", function (event) 
		camera_util.move(camera, 0, calc_step(speed_persecond, timer.delta * 0.001), 0)
	end)
	objctrller.bind_tigger("move_down", function (event) 
		camera_util.move(camera, 0, -calc_step(speed_persecond, timer.delta * 0.001), 0)
	end)
