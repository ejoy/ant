...
function geometry.cube_star(size, radius)
	radius = radius or 1
	local math3d = require "math3d"

	local cube_len<const> = math.sqrt(2);
	local h<const> = cube_len * 0.5;
	local tlf<const> = math3d.vector(-h, h, h)
	local trf<const> = math3d.vector( h, h, h)
	local tln<const> = math3d.vector(-h, h,-h)
	local trn<const> = math3d.vector( h, h,-h)
	local blf<const> = math3d.vector(-h,-h, h)
	local brf<const> = math3d.vector( h,-h, h)
	local bln<const> = math3d.vector(-h,-h,-h)
	local brn<const> = math3d.vector( h,-h,-h)

	local function create_face_vertices(pts, vertices, mirror_mask)
		local dl = math3d.mul(math3d.sub(pts[2], pts[1]), 1/size)
		local dt = math3d.mul(math3d.sub(pts[3], pts[1]), 1/size)
		local nv = (size+1)
		local fn = nv * nv * 3
		...
		for il=0, size do
			...
			local il_offset = offset + il * nv * 3
			for it=0, size do
	...
	local vertices = {}
	create_face_vertices({tlf, tln, trf}, vertices, {1, -1, 1})	--top/bottom
	create_face_vertices({blf, bln, tlf}, vertices, {-1, 1, 1})	--left/right
	create_face_vertices({tln, bln, trn}, vertices, {1, 1, -1})	--front/back

	local function create_face_indices()
		local indices = {}
	...
	local fn = (size+1) * (size+1)
	local indices = create_face_indices()
	local num = #indices
	for i=1, 5 do
		local offset = i * fn
		for j=1, num do
	...
	-- local face = {}
	-- local delta_t = math.pi / size
	-- local delta_f = math.pi * 2 / size
