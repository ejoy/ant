						...
						float gloss)
{
	...
	float specularFactor = pow(hdotn, shininess * 64.0);// * u_specularLight.x;

	vec3 diffuse = (diffuseColor * lightColor * ndotl).xyz;

	vec3 specular = specularColor.rbg * specularFactor * gloss;
	vec3 ambient = (diffuseColor * lightColor * 1.2).rgb;
...
void main()
{
	vec2 tt = vec2(v_tex0.x, 1.0-v_tex0.y);
