...
function EffectView:set_model(eid)
    ...
    if world[eid].effekseer then
        ...
        self.speed:set_getter(function() return world[eid].effekseer.speed end)
...
function EffectView:update()
    ...
    self.speed:update()
    ui_auto_play[1] = world[self.eid].effekseer.auto_play
    ui_loop[1] = world[self.eid].effekseer.loop
end
...
function EffectView:on_set_speed(value)
    local le = world[self.eid].effekseer
    le.speed = value
    effekseer.set_speed(le.handle, value)
end

function EffectView:on_set_auto_play(value)
    world[self.eid].effekseer.auto_play = value
end

function EffectView:on_set_loop(value)
    local le = world[self.eid].effekseer
    le.loop = value
    effekseer.set_loop(le.handle, value)
end
