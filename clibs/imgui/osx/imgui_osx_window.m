...
@interface WindowDelegate : NSObject<NSWindowDelegate> {
    uint32_t m_count;
    NSWindow* m_window;
    struct window_callback* m_cb;
}
- (id)init;
- (void)getMouseX:(int32_t*)outx getMouseY:(int32_t*)outy;
- (void)windowCreated:(NSWindow*)window initCallback:(struct window_callback*)callback ;
...
int window_init(struct window_callback* cb) {
    // do noting
    return 0;
}

int window_create(struct window_callback* cb, int w, int h) {
...
static bool dispatch_event(struct window_callback* cb, NSEvent* event) {
	if (!event) {
	    return false;
	}
...
void window_mainloop(struct window_callback* cb, int update) {
    if (!g_wd) {
        return;
    }
...
void window_exit(struct window_callback* cb) {
    //TODO
}
