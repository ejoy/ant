...
static inline void
matrix44_decompose2(const union matrix44 *m, struct vector3 *trans, struct vector3 *rot, struct vector3 *scale ) {
	...
	if (fabs(sp) > 0.9999f){
		rot->z = 0.f;
		rot->y = atan2(-m->c[0][2], m->c[0][0]);
	} else {
		rot->y = atan2(m->c[2][0], m->c[2][2]);
		rot->z = atan2(m->c[0][1], m->c[1][1]);
	}


	// rot->x = TO_DEGREE(rot->x);
	// rot->y = TO_DEGREE(rot->y);
	// rot->z = TO_DEGREE(rot->z);
}
