...
namespace ozzlua::RawAnimation {
	static int setup(lua_State* L) {
		...
		raw.v->duration = (float)lua_tonumber(L, 3);
		raw.v->tracks.resize(ske.v->num_joints());
		return 0;
	}
	...
	static int push_prekey(lua_State* L) {
		auto& raw = bee::lua::checkudata<ozzRawAnimation>(L, 1);
		auto& ske = bee::lua::checkudata<ozzSkeleton>(L, 2);
		int idx = ozz::animation::FindJoint(*ske.v, luaL_checkstring(L, 3));
		if (idx < 0) {
			luaL_error(L, "Can not found joint name");
			return 0;
		}
		ozz::animation::offline::RawAnimation::JointTrack& track = raw.v->tracks[idx];
	...
	static int build(lua_State* L) {
		...
		ozz::animation::Animation* animation = builder(*raw.v).release();
		if (!animation) {
	...
	static int clear_prekey(lua_State* L) {
		auto& raw = bee::lua::checkudata<ozzRawAnimation>(L, 1);
		auto& ske = bee::lua::checkudata<ozzSkeleton>(L, 2);
		int idx = ozz::animation::FindJoint(*ske.v, lua_tostring(L, 3));
		if (idx < 0) {
			luaL_error(L, "Can not found joint name");
			return 0;
		}
