...
local function rgb_2_saturation(rgb)
    -- Input:  ACES
    -- Output: OCES
...
local function darkSurround_to_dimSurround(linearCV)
    local XYZ = math3d.transform(AP1_to_XYZ, linearCV, 0)
    local xyY = XYZ_to_xyY(XYZ)

    local Y = math3d.index(xyY, 3)

    Y = clamp(Y, 0.0, 1e10)
    Y = Y ^ DIM_SURROUND_GAMMA
...
local RRT_GLOW_GAIN = 0.05
local RRT_GLOW_MID  = 0.08

-- Red modifier constants
local RRT_RED_SCALE = 0.82
local RRT_RED_PIVOT = 0.03
local RRT_RED_HUE   = 0.0
local RRT_RED_WIDTH = 135.0

-- Desaturation constants
local RRT_SAT_FACTOR = 0.96
local ODT_SAT_FACTOR = 0.93

local function ACES(color, brightness)
    -- Some bits were removed to adapt to our desired output
    local ap0 = math3d.transform(Rec2020_to_AP0, color, 0)

    -- Glow module
    local saturation = rgb_2_saturation(ap0)
    local ycIn = rgb_2_yc(ap0)
    ...
    -- Red modifier
    local hue = rgb_2_hue(ap0)
    local centeredHue = center_hue(hue, RRT_RED_HUE)
    local hueWeight = smoothstep(0.0, 1.0, 1.0 - math.abs(2.0 * centeredHue / RRT_RED_WIDTH))
    hueWeight = hueWeight * hueWeight;

    local ap0r = math3d.index(ap0, 1)
    ap0r = ap0r + hueWeight * saturation * (RRT_RED_PIVOT - ap0.r) * (1.0 - RRT_RED_SCALE);
    ap0r = math3d.set_index(ap0, ap0r)

    -- ACES to RGB rendering space
    local ap1 = mu.clamp(math3d.transform(AP0_to_AP1, ap0, 0), mc.ZERO, math3d.vector(1e10, 1e10, 1e10));

    -- Global desaturation
    local dot_luminance_ap1 = math3d.dot(ap1, LUMINANCE_AP1)
    ...
    local dot_cv_luminance = math3d.dot(linearCV, LUMINANCE_AP1)
    linearCV = math3d.lerp(math3d.vector(dot_cv_luminance), linearCV, ODT_SAT_FACTOR);

    return math3d.transform(AP1_to_Rec2020, linearCV, 0);
end
