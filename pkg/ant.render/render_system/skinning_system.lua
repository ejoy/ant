...
local skinning_matrices_vb
local skinning_in_dynamic_vb
local skinning_out_dynamic_vb
local function create_skinning_compute(vb_num)
	local dispatchsize = {
...
function skinning_sys:skin_mesh()
	...
	local meshskin
	local worldmat
	for e in w:select "skinning scene?in meshskin?in render_object?update bounding?update scene?in" do
		if e.meshskin then
			meshskin = e.meshskin
			...
			e.render_object.worldmat = worldmat
 			if skinning_matrices_vb == nil then
				...
				skinning_out_dynamic_vb = bgfx.create_dynamic_vertex_buffer(e.render_object.vb_num, declmgr.get "p40NIf|t40NIf|T40NIf".handle, "w")
				create_skinning_compute(e.render_object.vb_num)
...
function skinning_sys:render_preprocess()
