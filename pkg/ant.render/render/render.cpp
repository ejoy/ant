...
draw_indirect_obj(lua_State *L, struct ecs_world *w, bgfx_view_id_t viewid,
	...
	//TODO: material index should remove
	mesh_submit(w, ro, io, viewid, material_idx);
...
draw_obj(lua_State *L, struct ecs_world *w, bgfx_view_id_t viewid,
	...
	apply_material_instance(L, mi, w);
	//TODO: material index should remove
	mesh_submit(w, ro, nullptr, viewid, material_idx);
	...
	if (mats){
		for (int i=0; i<(int)mats->size()-1; ++i) {
			...
			w->bgfx->encoder_set_transform_cached(w->holder->encoder, t.tid, t.stride);
			w->bgfx->encoder_submit(w->holder->encoder, viewid, prog, ro->render_layer, BGFX_DISCARD_TRANSFORM|discardflags);
		}
		...
		for (uint8_t ii=0; ii<ctx->ra_count; ++ii){
			...
			for (uint16_t is=0; is<num; ++is){
				...
				if (obj_visible(ctx->w->Q, *so.ro, ra->queue_index)){
					...
					if (mi){
						if (so.io){
							draw_indirect_obj(ctx->L, ctx->w, ra->viewid, so.ro, so.io, mi, ra->material_index, so.prog, BGFX_DISCARD_ALL, trans);
						} else {
							draw_obj(ctx->L, ctx->w, ra->viewid, so.ro, mi, ra->material_index, so.prog, nullptr, BGFX_DISCARD_ALL, trans);
						}
					}
				}
			}
