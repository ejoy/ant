...
#define A_HLSL 1
    ...
    IMAGE2D_WO(s_image_output, rgba32f, 1);
        ...
        AF4 FsrEasuGF(AF2 p) { AF4 res = textureGather(s_image_input, p, 1); return res; }
        AF4 FsrEasuBF(AF2 p) { AF4 res = textureGather(s_image_input, p, 2); return res; }
    #endif
    #if SAMPLE_RCAS
        #define FSR_RCAS_F
        AF4 FsrRcasLoadF(ASU2 p) { return texelFetch(s_image_input, ASU2(p), 0); }
    ...
    IMAGE2D_WO(s_image_output, rgba16f, 1);
        ...
        AH4 FsrEasuGH(AF2 p) { AH4 res = AH4(textureGather(s_image_input, p, 1)); return res; }
        AH4 FsrEasuBH(AF2 p) { AH4 res = AH4(textureGather(s_image_input, p, 2)); return res; }
    #endif
    #if SAMPLE_RCAS
        #define FSR_RCAS_H
        AH4 FsrRcasLoadH(ASW2 p) { return AH4(texelFetch(s_image_input, ASU2(p), 0)); }
...
void CurrFilter(AU2 pos, AU4 Const0, AU4 Const1, AU4 Const2, AU4 Const3, AU4 Sample)
{
    ...
    imageStore(s_image_output, ASU2(pos), texture2DLod(s_image_input, pp, 0.0));
#endif
#if SAMPLE_EASU
    #if SAMPLE_SLOW_FALLBACK
        AF3 c;
        ...
        if( Sample.x == 1 )
            c *= c;
        imageStore(s_image_output, ASU2(pos), AF4(c, 1));
    #else
        AH3 c;
        FsrEasuH(c, pos, Const0, Const1, Const2, Const3);
        if( Sample.x == 1 )
            c *= c;
        imageStore(s_image_output, ASU2(pos), AH4(c, 1));
    #endif
#endif
#if SAMPLE_RCAS
    #if SAMPLE_SLOW_FALLBACK
