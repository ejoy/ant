...
local SceneHierarchyHub = require "scene_hierarchy_hub"
...
local function start_watch_entities(eids)
    ...
    hub.publish(WatcherEvent.SEND_ENTITIES,setialize_result)
end
...
function editor_watcher_system:init()
    ...
    hub.subscibe(WatcherEvent.WATCH_ENTITIES,start_watch_entities)
    hub.subscibe(WatcherEvent.MODIFY_COMPONENT,on_component_modified)
    hub.subscibe(WatcherEvent.REQUEST_WORLD_INFO,publish_world_info)
    publish_world_info()
end
