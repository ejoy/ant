...
if cfg.gen_tex == 1 then 
    ...
    for i= 1 , NumTextures do   
        ...
        if string.find(target_texture,"terrain_grass_01_a") then 
            print("check")
        end 
        ...
        local commandline = command..'"'..PROJECT_DIR..world[1].Textures[i]..'"'..' '.."-quality 100"..' '..'"'..OUT_DIR..target_texture..'"'
        print(commandline)
...
function GetMatProperties(Textures,MetaData)
    ...
    if MetaData._BaseColorMap0 ~= nil or MetaData._BaseColor0 ~=nil then 
        ...
        if MatProp.BaseColorMap == nil  then 
           MatProp.BaseColorMap = Textures[MetaData._MainTex]    
        end 
        if MatProp.NormalMap == nil then 
           MatProp.NormalMap    = Textures[MetaData._BumpMap]
        end 
        if MatProp.MaskMap == nil then 
           MatProp.MaskMap      = Textures[MetaData._SpecGlossMap]
        end 

        if MatProp.MaskMap then 
            MatProp.GlossMapScale = MetaData._GlossMapScale
        end 
        ...
        -- end 
        if MatProp.BaseColor == nil  then 
            MatProp.BaseColor = MetaData._Color 
        end 
        if MatProp.Metallic == nil then 
            MatProp.Metallic = 0        
        end        
        if MetaData._SpecColor then 
            MatProp.SpecColor = MetaData._SpecColor 
            MatProp.Smoothness = MetaData._Glossiness 
        end 
        ...
        if MatProp.GlossMapScale == nil then MatProp.GlossMapScale= 0.5 end 

        MatProp.Smoothness = MatProp.Smoothness 

        MatProp.Mask = MatProp._Mask 
        if MetaData._MainTexTiling then 
            MatProp.MainTexTiling = {MetaData._MainTexTiling[1],MetaData._MainTexTiling[2],0,0}
        else
            MatProp.MainTexTiling = {1,1,0,0} 
        end 
...
for i= 1, NumMaterials do  
    ...
    if string.find(MetaData.Name,"mat_plank_01") then 
        print("check")
    end 
    ...
    BrdfMap      = package_path.."assets/textures/brdfMap3.texture"
    --texdesc_name = texdesc_name..TARGET_TEXDESC_TYPE            --".texture"
    ...
    ..texture_def('s_brdfMap',10,BrdfMap)
    ..'  },\n'
    ..'  uniforms = {\n'
    ...
    ..uniform_def('u_tiling', MatProp.MainTexTiling )
    ..'  },\n'
