...
function PVPScene.create_entitices(world)	
	local scene_objects = {
		CampsiteDoor = {			
			srt = mu.srt({1, 1, 1, 0}, nil, {-12.95, 0.7867187, -14.03104}),
	...
	for name, scenedata in pairs(scene_objects) do
		...
		for idx_array=1, #srts do			
			...
			for idx=1, #srt_array do
				...
				local s = srt.s or scenedata.srt.s
				local r = to_radian(srt.r or scenedata.srt.r)
				local t = srt.t or scenedata.srt.t

				local rsrt = scenedata.relate_srts and scenedata.relate_srts[idx_array] or nil
				if rsrt then
					s = ms(s, rsrt.s, "+T")
					r = ms(r, to_radian(rsrt.r), "+T")
					t = ms(t, rsrt.t, "+T")
				end
