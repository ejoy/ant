...
local function gen_color_vertices(pts, color, transform, vb, ms)
	local vnum = #pts
	if transform then
		for i=1, vnum do
			table.insert(vb, gen_color_vertex(pts[i], color, transform, ms))
		end
	else
		for i=1, vnum do
			local pt = pts[i]
...
function draw.draw_line(pts, color, transform, ms, desc)
	local vb = assert(desc.vb)

	local offset = #vb
	local num = #pts
	gen_color_vertices(pts, color, transform, vb, ms)
	add_primitive(desc.primitives, offset, num)
end

local function draw_primitve(color, transform, ms, desc, buffer_generator)
	...
	gen_color_vertices(vb, color, transform, desc_vb, ms)
...
function draw.draw_cone(cone, color, transform, ms, desc)
	draw_primitve(color, transform, ms, desc, 
	function()
		return geo.cone(10, cone.height, cone.radius, true, true)
	end)
end

function draw.draw_sphere(sphere, color, transform, ms, desc)
	draw_primitve(color, transform, ms, desc, function()
